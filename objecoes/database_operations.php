...
function editObjecao($pdo, $id, $titulo, $prompt, $intencao) {
    try {
...
function addObjecao($pdo, $titulo, $prompt, $intencao) {
    try {
        $stmt = $pdo->prepare("INSERT INTO fases (titulo, prompt, tipo, sessao, intencao) VALUES (?, ?, 'OBJECAO', ?, ?)");
...
function getObjecoes($pdo, $intencao = null) {
    try {
        $params = [$_SESSION['user_id']];
        $sql = "SELECT * FROM fases WHERE tipo = 'OBJECAO' AND sessao = ?";
