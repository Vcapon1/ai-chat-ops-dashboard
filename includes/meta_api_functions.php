...
function buscarTemplatesMeta($whats_id, $access_token) {
    $api_url = "https://graph.facebook.com/v21.0/{$whats_id}/message_templates";
    ...
    $ch = curl_init($api_url . "?status=APPROVED&limit=50");
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, 30);
    ...
    error_log("Buscar Templates Meta - HTTP Code: {$http_code}");
    error_log("Buscar Templates Meta - Response: {$response}");
    ...
    return [
        'success' => false,
        'message' => 'Erro ao buscar templates da API Meta',
        'http_code' => $http_code
    ];
}
