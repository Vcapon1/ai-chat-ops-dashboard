...
function processarEnviosCampanhaMeta($campanha) {
    ...
    try {
        ...
        $templates = [];
        if (!empty($campanha['templates_selecionados'])) {
            $templates = json_decode($campanha['templates_selecionados'], true);
            if (!is_array($templates) || empty($templates)) {
                error_log("Campanha {$campanha['id']}: Nenhum template válido configurado");
                return;
            }
        }
        ...
        foreach ($contatos as $contato) {
            ...
            // Escolher template aleatório da lista
            $template_escolhido = $templates[array_rand($templates)];
            $template_name = $template_escolhido['name'];
            $template_body_text = $template_escolhido['body_text'] ?? 'Template sem texto';
            
            // Extrair parâmetros se houver na mensagem original
            ...
            if ($sucesso) {
                ...
                // Atualizar com template usado e texto do template
                $stmt_update = $pdo_disparador->prepare("
                    UPDATE `{$table_name}` 
                    SET enviado = 1, data_envio = NOW(), mensagem = ? 
                    WHERE id = ?
                ");
                $mensagem_salvar = "Template: {$template_name}\n\n{$template_body_text}";
                $stmt_update->execute([$mensagem_salvar, $contato['id']]);
                $enviados++;
                
                error_log("Mensagem Meta enviada com sucesso para: " . $contato['telefone'] . " (Template: {$template_name})");
            } else {
