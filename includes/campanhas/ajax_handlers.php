...
function handleCreateCampanha() {
    ...
    try {
        ...
        $templates_selecionados_raw = $_POST['templates_selecionados'] ?? '[]';
        
        // Decodificar templates selecionados
        $templates_selecionados = json_decode($templates_selecionados_raw, true);
        if (!is_array($templates_selecionados)) {
        ...
        // Validar se há templates selecionados ou mensagem personalizada
        if (empty($templates_selecionados) && empty($mensagem)) {
            throw new Exception('Selecione pelo menos um template WhatsApp');
        }
        ...
        // Criar campanha com novos campos incluindo tom_voz e templates
        if (criarCampanhaAvancada($pdo_disparador, $id_session, $nome_campanha, $id_lista, $mensagem, 
                                  $data_inicio, $hora_inicio, $hora_fim, $dias_array, $usar_ia, $contexto_empresa, $tom_voz, $templates_selecionados)) {
...
function handleGetMetaTemplates() {
    try {
        require_once '../db.php';
        ...
        // Buscar configuração Meta API
        $stmt = $pdo->prepare("SELECT whatsapp_business_account_id, access_token FROM integrators_meta WHERE id_session = ?");
        $stmt->execute([$id_session]);
        $meta_config = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$meta_config) {
            echo json_encode(['success' => false, 'message' => 'Configuração Meta API não encontrada. Configure em Integrações.']);
            return;
        }
        
        if (empty($meta_config['whatsapp_business_account_id']) || empty($meta_config['access_token'])) {
            echo json_encode(['success' => false, 'message' => 'Configuração Meta API incompleta. Verifique whatsapp_business_account_id e access_token.']);
            return;
        }
        
        $result = buscarTemplatesMeta($meta_config['whatsapp_business_account_id'], $meta_config['access_token']);
        
        if ($result['success']) {
            // Processar templates para extrair texto do body
            $templates_processados = array_map(function($template) {
                $body_text = '';
            ...
            echo json_encode(['success' => true, 'templates' => $templates_processados]);
        } else {
            echo json_encode($result);
        }
        
    } catch (Exception $e) {
        error_log("Erro ao buscar templates Meta: " . $e->getMessage());
