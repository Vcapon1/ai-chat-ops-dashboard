...
document.getElementById('form-campanha').addEventListener('submit', function(e) {
    ...
    // Validar contexto da empresa se IA estiver ativada
    const usarIA = document.getElementById('usar_ia').checked;
    if (usarIA) {
        const contextoEmpresa = document.getElementById('contexto_empresa').value.trim();
        if (!contextoEmpresa) {
            showNotification('Contexto da empresa é obrigatório quando usar IA', 'error');
            return;
        }
    }
