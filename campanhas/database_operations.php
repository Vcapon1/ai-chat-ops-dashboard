...
function criarTabelaEnviosCampanha($pdo_disparador, $id_session, $id_campanha, $id_lista, $mensagem) {
    try {
        ...
        $check_stmt = $pdo_disparador->prepare("
            ...
            WHERE b.telefone IS NULL AND c.whatsapp_validado = 1 AND c.whatsapp_ativo = 1
        ");
        $check_stmt->execute([$id_session]);
        $contatos_disponiveis = $check_stmt->fetchColumn();
        
        error_log("DEBUG: Lista {$id_lista} tem {$contatos_disponiveis} contatos disponíveis (validados e ativos, não blacklist)");
        
        if ($contatos_disponiveis == 0) {
            throw new Exception('A lista não possui contatos validados e ativos para envio. Valide os números WhatsApp antes de criar a campanha.');
        }
        ...
        // Copiar contatos da lista para a tabela de envios, excluindo blacklist e incluindo apenas contatos validados e ativos
        $insert_sql = "
            INSERT INTO `{$table_name}` (" . implode(', ', $insert_columns) . ")
            SELECT " . implode(', ', $select_columns) . "
            FROM `{$contatos_table}` c
            LEFT JOIN dis_blacklist_optout b ON c.telefone = b.telefone AND b.id_session = ?
            WHERE b.telefone IS NULL AND c.whatsapp_validado = 1 AND c.whatsapp_ativo = 1 AND c.id_lista = ?
        ";
