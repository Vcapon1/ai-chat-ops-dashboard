...
require_once 'includes/duvidas/database_operations.php';
require_once 'includes/duvidas/table_view.php';
require_once 'includes/duvidas/add_form.php';
...
// Get the selected intention
$selectedIntencao = isset($_GET['intencao']) ? $_GET['intencao'] : '0';

// Processar requisições POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete']) && isset($_POST['id'])) {
        if (deleteDuvida($pdo, $_POST['id'])) {
            header('Location: duvidas-config.php' . ($selectedIntencao != '0' ? '?intencao=' . $selectedIntencao : ''));
            exit;
        }
    }
    
    if (isset($_POST['edit'])) {
        if (editDuvida($pdo, $_POST['id'], $_POST['titulo'], $_POST['prompt'], $_POST['intencao'])) {
            header('Location: duvidas-config.php' . ($selectedIntencao != '0' ? '?intencao=' . $selectedIntencao : ''));
            exit;
        }
    }
    ...
    if (isset($_POST['add'])) {
        if (addDuvida($pdo, $_POST['titulo'], $_POST['prompt'], $_POST['intencao'])) {
            header('Location: duvidas-config.php' . ($selectedIntencao != '0' ? '?intencao=' . $selectedIntencao : ''));
            exit;
        }
    }
...
// Buscar dúvidas
$result = getDuvidas($pdo, $selectedIntencao);
...
<div class="flex min-h-screen bg-gray-900">
    ...
    <div class="flex-1 p-8">
        <div class="max-w-4xl mx-auto">
            <h1 class="text-3xl font-bold text-white mb-8">Configuração de Dúvidas</h1>
            
            <!-- Navigation Buttons -->
            <div class="mb-8">
                <?php renderNavigationButtons('duvidas'); ?>
            </div>
            ...
            <div id="promptModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-gray-800 p-6 rounded-lg w-full max-w-2xl">
                    <h2 class="text-xl font-bold text-white mb-4">Editar Dúvida</h2>
